...
<head>
    ...
    <style>
        ...
        .card-form { border: none; border-radius: 24px; box-shadow: 0 10px 40px rgba(0,0,0,0.03); background: white; transition: 0.3s; }
        .section-title { font-size: 1.1rem; font-weight: 700; color: #1e293b; margin-bottom: 1.5rem; display: flex; align-items: center; }
        .section-title i { color: #34495e; margin-right: 12px; }
        .form-label { font-weight: 700; color: #64748b; font-size: 0.75rem; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 8px; }
        .form-control, .form-select { border-radius: 12px; padding: 12px 16px; border: 1px solid #e2e8f0; background-color: #fcfdfe; font-size: 0.95rem; }
        .form-control.is-invalid, .form-select.is-invalid { border-color: #ef4444 !important; background-color: #fef2f2 !important; }
        #map { width: 100%; height: 400px; border-radius: 16px; border: 2px solid #f1f5f9; z-index: 1; }
        .bg-readonly { background-color: #f1f5f9 !important; font-weight: 600; color: #475569; }
        .btn-submit { background: #34495e; border: none; padding: 16px 30px; border-radius: 14px; font-weight: 700; transition: 0.3s; color: white; }
...
let marker = L.marker([-7.022, 107.518], {draggable:true}).addTo(map);
...
map.on('click', (e) => { marker.setLatLng(e.latlng); updateLatLng(e.latlng.lat, e.latlng.lng); });
...
document.getElementById('formReses').addEventListener('submit', function(e) {
    ...
    if (!document.getElementById('lat').value) {
        Swal.fire({ icon: 'warning', title: 'Lokasi?', text: 'Klik peta dulu!' });
        return;
    }
